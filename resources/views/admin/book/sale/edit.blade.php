    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <select name="bookId" id="" class="form-control">
            @foreach ($totalBooks as $book)
                <option value="{{$book->id}}" {{$book->id == $data->bookId ? 'selected' : ''}}>{{$book->name}}</option>
            @endforeach
        </select>
        <label for="">Sale</label>
        <input type="number" name="salePercent" value="{{$data->salePercent}}" class="form-control" onKeyPress="if(this.value.length==2) return false;" min="1" max="99" required>
